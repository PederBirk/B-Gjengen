...
def loadData(symbols, nTrainingData, nTestData, pathToData): # TODO validation data?
    data = { 'train': [], 'test': [] }
    for symbol in symbols:
        ...
        for file in listdir(path):
            if trainingCounter < nTrainingData/len(symbols):
                data['train'].append((readImage(pathToData + symbol + '/'  + file), expectedOutput))
                trainingCounter += 1
            elif testCounter < nTestData/len(symbols):
                data['test'].append((readImage(pathToData + symbol + '/'  + file), expectedOutput))
                testCounter += 1
            else:
                break
    return data
